...
use crate::glbackend::types::{
    GLTypeBuffer, GLTypeProgram, GLTypeShader, GLTypeTexture, GLTypeUniformLocation, GLenum, GLint,
    GLuint,
};
...
fn gl_check_error_always(context: &GLContextManager) -> Result<(), BacktraceError<ErrorMessage>> {
...
impl GLHandleId for GLHandleTypeProgram {
    ...
    fn delete(&self, context: &GLContextManager) {
        context.with_gl_backend(|backend| unsafe { backend.gl_delete_program(self.handle) });
    }
}
...
impl GLHandleId for GLHandleTypeShader {
    ...
    fn delete(&self, context: &GLContextManager) {
        context.with_gl_backend(|backend| unsafe { backend.gl_delete_shader(self.handle) });
    }
}
...
impl GLHandleId for GLHandleTypeBuffer {
    ...
    fn delete(&self, context: &GLContextManager) {
        context.with_gl_backend(|backend| unsafe { backend.gl_delete_buffer(self.handle) });
    }
}
...
impl GLHandleId for GLHandleTypeTexture {
    ...
    fn delete(&self, context: &GLContextManager) {
        context.with_gl_backend(|backend| unsafe { backend.gl_delete_texture(self.handle) });
    }
}
...
impl GLUniformHandle {
    pub fn set_value_float(&self, context: &GLContextManager, value: f32) {
        context.with_gl_backend(|backend| unsafe { backend.gl_uniform_1f(&self.handle, value) })
    }

    pub fn set_value_int(&self, context: &GLContextManager, value: i32) {
        context.with_gl_backend(|backend| unsafe { backend.gl_uniform_1i(&self.handle, value) })
    }
}
...
impl GLTexture {
    ...
    pub fn set_image_data(
        ...
        context.with_gl_backend::<Result<(), BacktraceError<ErrorMessage>>, _>(|backend| unsafe {
            backend.gl_pixel_store_i(GL_UNPACK_ALIGNMENT, unpack_alignment);
            backend.gl_tex_parameter_i(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE as GLint);
            backend.gl_tex_parameter_i(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE as GLint);
            backend.gl_tex_parameter_i(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, smoothing_constant);
            backend.gl_tex_parameter_i(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, smoothing_constant);
...
#[must_use]
fn obtain_context_if_valid(state: &RefCell<GLContextManagerState>) -> Option<GLContextManager> {
...
impl GLContextManager {
    pub fn create(
        ...
        RefCell::borrow_mut(&manager.state).weak_ref_to_self = Rc::downgrade(&manager.state);
