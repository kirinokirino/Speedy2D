...
impl<E: Debug + Display> BacktraceError<E> {
    ...
    #[must_use]
    pub(crate) fn context<S: AsRef<str>>(self, description: S) -> BacktraceError<ErrorMessage> {
...
impl ErrorMessage {
    ...
    pub(crate) fn msg_with_cause<S, Cause>(description: S, cause: Cause) -> BacktraceError<Self>
    where
        S: AsRef<str>,
...
pub(crate) trait Context<R> {
    fn context<S: AsRef<str>>(self, description: S) -> Result<R, BacktraceError<ErrorMessage>>;
}
