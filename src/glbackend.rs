...
pub mod constants {
    ...
    pub const GL_INVALID_FRAMEBUFFER_OPERATION: GLenum = glow::INVALID_FRAMEBUFFER_OPERATION;
...
pub trait GLBackend {
    ...
    unsafe fn gl_create_program(&self) -> Result<GLTypeProgram, BacktraceError<ErrorMessage>>;
    ...
    unsafe fn gl_gen_texture(&self) -> Result<GLTypeTexture, BacktraceError<ErrorMessage>>;

    #[must_use]
    unsafe fn gl_get_error(&self) -> GLenum;

    #[must_use]
    unsafe fn gl_get_attrib_location(&self, program: GLTypeProgram, name: &str) -> Option<GLuint>;
...
impl GLBackend for GLBackendGlow {
    ...
    unsafe fn gl_create_program(&self) -> Result<GLTypeProgram, BacktraceError<ErrorMessage>> {
        let handle = self
            .context
            .create_program()
            .map_err(|err| ErrorMessage::msg(format!("Failed to create program: {err}")))?;
    ...
    unsafe fn gl_create_shader(
        ...
        let handle = self
            ...
            .map_err(|err| ErrorMessage::msg(format!("Failed to create shader: {err}")))?;
    ...
    unsafe fn gl_gen_buffer(&self) -> Result<GLTypeBuffer, BacktraceError<ErrorMessage>> {
        let handle = self
            .context
            .create_buffer()
            .map_err(|err| ErrorMessage::msg(format!("Failed to create buffer: {err}")))?;
    ...
    unsafe fn gl_gen_texture(&self) -> Result<GLTypeTexture, BacktraceError<ErrorMessage>> {
        let handle = self
            .context
            .create_texture()
            .map_err(|err| ErrorMessage::msg(format!("Failed to create texture: {err}")))?;
...
impl GLBackend for GLBackendGLRS {
    ...
    unsafe fn gl_get_string(&self, parameter: u32) -> String {
        String::from_utf8_lossy(CStr::from_ptr(gl::GetString(parameter) as *const _).to_bytes())
            .to_string()
    }
...
#[cfg(not(target_arch = "wasm32"))]
impl GLBackendGLRS {
    fn gl_call_get_info_log<F>(&self, callback: F) -> Result<String, BacktraceError<ErrorMessage>>
    where
        F: FnOnce(GLsizei, *mut GLsizei, *mut GLchar) -> Result<(), BacktraceError<ErrorMessage>>,
    {
