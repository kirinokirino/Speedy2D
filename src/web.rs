...
impl WebWindow {
    pub fn new() -> Result<Self, BacktraceError<ErrorMessage>> {
        Ok(Self {
            window: web_sys::window().ok_or_else(|| ErrorMessage::msg("Failed to get window"))?,
        })
    }
    ...
    #[cfg(feature = "windowing")]
    pub fn match_media(&self, query: &str) -> Result<WebEventTarget, BacktraceError<ErrorMessage>> {
    ...
    #[cfg(feature = "windowing")]
    pub fn dyn_into_event_target(self) -> Result<WebEventTarget, BacktraceError<ErrorMessage>> {
...
impl WebElement {
    pub fn dyn_into_html_element(self) -> Result<WebHtmlElement, BacktraceError<ErrorMessage>> {
...
impl WebHtmlElement {
    ...
    pub fn dyn_into_canvas(self) -> Result<WebCanvasElement, BacktraceError<ErrorMessage>> {
        let canvas = self
            .html_element
            .clone()
            .dyn_into::<HtmlCanvasElement>()
            .map_err(|err| {
                ErrorMessage::msg(format!("Failed to convert element to canvas: '{err:?}'"))
            })?;
...
impl WebCanvasElement {
        ...
        V: Into<UVec2>,
    {
        ...
        let context = self
            ...
            .get_context("webgl2")
            .map_err(|err| {
                GLRendererCreationError::msg(format!("Failed to get WebGL2 context: '{err:?}'"))
            })?
...
impl WebPending {
    ...
    fn new_with_status<F: FnOnce(WebPendingStatus) + 'static>(unregister_action: F) -> Self {
        Self {
...
#[cfg(feature = "windowing")]
impl WebEventTarget {
    fn dyn_from<E: Debug + JsCast>(element: E) -> Result<Self, BacktraceError<ErrorMessage>> {
        Ok(WebEventTarget {
    ...
    pub fn register_event_listener_void<F: FnMut() + 'static>(
        ...
        self.register_event_listener(listener_type, Box::new(callback) as Box<dyn FnMut()>, false)
    }
    ...
    >(
        ...
        self.register_event_listener(listener_type, Box::new(callback) as Box<dyn FnMut(_)>, true)
    }
